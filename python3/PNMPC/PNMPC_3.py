...
#toy_esn = RNN.EchoStateNetwork(neu = 100,n_in = 2,n_out = 3,gama = 0.14,ro = 0.99,psi = 0.0,in_scale = 0.1,bias_scale = 0.1, initial_filename = "initial", load_initial = True, save_initial = False,output_feedback = False)
#-----------------------------------------------------------------------------------------------
...
#offline_training(self,X,Y,regularization,warmupdrop):
toy_esn.offline_training(u_data_normal,y_data_normal,regularization,warmupdrop)

#print("TEST")

#error, reg = toy_esn.cum_train_cv(min_reg,max_reg)
...
u_max = np.max(some_input,axis=0)
u_min = np.min(some_input,axis=0)
