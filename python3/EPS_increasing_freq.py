...
valve = np.linspace(1,1,10)
frequency = [40,42,44,46,48,50,52,54,56,58]
...
x0 = vertcat(90e5,22e5,0.01)
p0 = vertcat(valve[0],frequency[0])
res = I(x0=x0,p=p0)
final_1 = res['xf'].full().T
temp = final_1[len(final_1)-1]

x0 = vertcat(temp)
p0 = vertcat(valve[1],frequency[1])
res = I(x0=x0,p=p0)
...
for i in range(2,10):
    ...
    x0 = vertcat(temp)
    p0 = vertcat(valve[i],frequency[i])
    res = I(x0=x0,p=p0)
    final_2 = res['xf'].full().T
    temp = final_2[len(final_1)-1]
...
plt.subplot(414)
plt.step(time_1,frequency,color='red',label='Frequency')
plt.grid()
plt.legend()
plt.ylabel('$f \ [Hz]$')
plt.xlabel('Time [min]')
